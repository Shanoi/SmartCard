...
static void read(void)
{
		...
		0x00, 0xA4, 0x00, 0x0C, 0x02, NDEF_File[0], NDEF_File[1]))
	{
		...
		*NDEF_data++ = io_data[2];
		...
		// Length:5; CLA:00 INS:B0 P1/P2:Offset Lc:- Data:- Le:MLe
		if (!nfc_forum_type_4_command_varargs(READ_BINARY, "NDEF", &result, &length, io_data, 5,
			0x00, 0xB0, (byte)(offset >> 8), (byte)offset, valid_content - offset + 2))
		{
			return;
		}
		else if (io_data[0] == 0x03 && io_data[1] == LE_INCORRECT[0] && io_data[2] == LE_INCORRECT[1])
		...
		copy_string(io_data, NDEF_data, valid_content - offset + 2, 1);
		NDEF_data -= offset;

		printf("NDEF Data:\n");
		display_data_string_to_hex(NDEF_data, valid_content + 2);
...
static void write(byte* data, unsigned int data_length)
{
		...
		0x00, 0xA4, 0x00, 0x0C, 0x02, NDEF_File[0], NDEF_File[1]))
	{
		...
		// 006291010A55016170706C652E636F6D110114540266724C612062656C6C6520686973746f697265110008504F4C595445434811010A55016170706C652E636F6D110114540266724C612062656C6C6520686973746f697265510008504F4C5954454348 //Correct data
